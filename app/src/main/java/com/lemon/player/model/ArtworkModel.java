    ...
    public static String getTypeString(@ArtworkProvider.Type int type) {
        switch (type) {
            case ArtworkProvider.Type.MEDIA_STORE:
                return LemonApplication.getInstance().getString(R.string.artwork_type_media_store);
            case ArtworkProvider.Type.TAG:
                return LemonApplication.getInstance().getString(R.string.artwork_type_tag);
            case ArtworkProvider.Type.FOLDER:
                return "Folder";
            case ArtworkProvider.Type.LAST_FM:
                return LemonApplication.getInstance().getString(R.string.artwork_type_lastfm);
            case ArtworkProvider.Type.ITUNES:
                return LemonApplication.getInstance().getString(R.string.artwork_type_itunes);
        }
        return null;
    }
