...
public class Playlist implements Serializable {
    ...
    public Playlist(Cursor cursor) {
        ...
        if (LemonApplication.getInstance().getString(R.string.fav_title).equals(name)) {
            type = Type.FAVORITES;
            canDelete = false;
            canRename = false;
        }
    ...
    public static Playlist podcastPlaylist() {
        ...
        return SqlUtils.createSingleQuery(LemonApplication.getInstance(), cursor -> new Playlist(
                        Type.PODCAST, PlaylistUtils.PlaylistIds.PODCASTS_PLAYLIST,
                        LemonApplication.getInstance().getString(R.string.podcasts_title),
    ...
    public static Playlist recentlyAddedPlaylist = new Playlist(
            ...
            LemonApplication.getInstance().getString(R.string.recentlyadded),
            false,
            false,
            false,
            false,
    ...
    public static Playlist mostPlayedPlaylist = new Playlist(
            ...
            LemonApplication.getInstance().getString(R.string.mostplayed),
            false,
            true,
            false,
            false,
    ...
    public static Playlist recentlyPlayedPlaylist = new Playlist(
            ...
            LemonApplication.getInstance().getString(R.string.suggested_recent_title),
            false,
            false,
            false,
            false,
    ...
    public boolean moveSong(int from, int to) {
        return MediaStore.Audio.Playlists.Members.moveItem(LemonApplication.getInstance().getContentResolver(), id, from, to);
    }

    public Observable<List<Song>> getSongsObservable() {

        if (id == PlaylistUtils.PlaylistIds.RECENTLY_ADDED_PLAYLIST) {
            int numWeeks = MusicUtils.getIntPref(LemonApplication.getInstance(), "numweeks", 2) * (3600 * 24 * 7);
            ...
            return SqlBriteUtils.createObservableList(LemonApplication.getInstance(), cursor ->
                    new Pair<>(
            ...
            return SqlBriteUtils.createObservableList(LemonApplication.getInstance(), Playlist::createSongFromPlaylistCursor, query)
                    .map(songs -> {
                        Collections.sort(songs, (a, b) -> ComparisonUtils.compareLong(a.playlistSongPlayOrder, b.playlistSongPlayOrder));
                        return songs;
                    });
