...
public class MusicService extends Service {
    ...
    void prepareChromeCastLoad(int position, boolean autoPlay) {
        ...
        metadata.addImage(new WebImage(Uri.parse("http://" + LemonUtils.getIpAddr() + ":5000" + "/image/" + getSongId())));

        MediaInfo selectedMedia = new MediaInfo.Builder("http://" + LemonUtils.getIpAddr() + ":5000" + "/audio/" + getSongId())
                .setStreamType(MediaInfo.STREAM_TYPE_BUFFERED)
                .setContentType("audio/*")
                .setMetadata(metadata)
                .build();

        if (LemonUtils.isUpgraded() && castManager != null) {
    ...
    private void setupMediaSession() {
        mediaSession = new MediaSessionCompat(this, "Lemon", mediaButtonReceiverComponent, null);
        ...
        if (what.equals(InternalIntents.TRACK_ENDING)) {
            ...
            if (finishedSong != null) {
                if (finishedSong.hasPlayed()) {
                    Completable.fromAction(() -> LemonUtils.incrementPlayCount(this, finishedSong)).subscribeOn(Schedulers.io())
                            .subscribe(() -> {
                                // Nothing to do
