...
class LyricsPresenter extends Presenter<LyricsView> {
    ...
    public void bindView(@NonNull LyricsView view) {
        ...
        addDisposable(RxBroadcast.fromBroadcast(LemonApplication.getInstance(), new IntentFilter(MusicService.InternalIntents.META_CHANGED))
    ...
    private void updateLyrics() {
        ...
        addDisposable(Observable.fromCallable(() -> {
            ...
            if (path.startsWith("content://")) {
                ...
                Cursor cursor = SqlUtils.createQuery(LemonApplication.getInstance(), query);
                if (cursor != null) {
                    try {
