...
public class SafManager {
    ...
    public boolean requiresPermission(File file) {
        return LemonUtils.hasKitKat() && getExtSdCardFolder(file) != null;
    }
    ...
    public DocumentFile getDocumentFile(final File file) {
        ...
        DocumentFile document = DocumentFile.fromTreeUri(LemonApplication.getInstance(), Uri.parse(treeUri));
    ...
    public String getDocumentTree() {
        ...
        List<UriPermission> perms = LemonApplication.getInstance().getContentResolver().getPersistedUriPermissions();
    ...
    private List<String> getExtSdCardPaths() {
        ...
        try {
            File[] externalFilesDirs = LemonApplication.getInstance().getExternalFilesDirs("external");
            if (externalFilesDirs != null && externalFilesDirs.length > 0) {
                for (File file : externalFilesDirs) {
                    if (file != null && !file.equals(LemonApplication.getInstance().getExternalFilesDir("external"))) {
                ...
                switch (requestCode) {
                    case DOCUMENT_TREE_REQUEST_CODE:
                        if (resultCode == Activity.RESULT_OK) {
                            ...
                            if (treeUri != null) {
                                LemonApplication.getInstance().getContentResolver().takePersistableUriPermission(treeUri, Intent.FLAG_GRANT_READ_URI_PERMISSION | Intent.FLAG_GRANT_WRITE_URI_PERMISSION);
