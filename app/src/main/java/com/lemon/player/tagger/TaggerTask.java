        ...
        for (int i = 0; i < paths.size(); i++) {
            ...
            try {
                ...
                if (tagUpdate.hasChanged()) {
                    ...
                    if (LemonUtils.hasLollipop() && TaggerUtils.requiresPermission(paths)) {
                        temp = new File(LemonApplication.getInstance().getFilesDir(), orig.getName());
                        tempFiles.add(temp);
                    ...
                    if (LemonUtils.hasLollipop() && requiresPermission && temp != null) {
                        DocumentFile documentFile = documentFiles.get(i);
                        if (documentFile != null) {
                            ParcelFileDescriptor pfd = LemonApplication.getInstance().getContentResolver().openFileDescriptor(documentFile.getUri(), "w");
                            if (pfd != null) {
